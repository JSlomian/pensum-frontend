    ...
    <div class="space-y-6">
      ...
      <div class="grid md:grid-cols-2 md:gap-6">
        <div class="relative z-0 w-full mb-5 group">
          ...
          >
            <template v-if="major?.member && major?.member?.length > 0">
              <option v-for="maj in major?.member as Major[]" :key="maj['@id']" :value="maj['@id']">
                {{ maj.name }}
              </option>
            </template>
          </select>
        ...
        <div class="relative z-0 w-full mb-5 group">
          ...
          <select
            id="underline_select"
            v-model="educationLevelIri"
            required
            class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer"
          >
            <template v-if="educationLevel?.member && educationLevel?.member?.length > 0">
              <option v-for="el in educationLevel?.member" :key="el['@id']" :value="el['@id']">
                {{ el.name }}
              </option>
            </template>
          </select>
