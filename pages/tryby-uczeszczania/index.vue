<script setup lang="ts">
  ...
  const handleDelete = async (id: number): Promise<void> => {
    const mode: AttendanceMode | undefined = data.value?.member.find(
      (m: AttendanceMode) => m.id === id
    )
    if (!mode) {
    ...
    await useDelete(route).callDelete(mode.id, {
  ...
  const handleUpdate = async (mode: AttendanceMode): Promise<void> => {
    await useUpdate(route).callUpdate(mode, {
      onResponse({ response }: { response: Response }) {
        if (response.ok) {
          handleCancelEdit()
